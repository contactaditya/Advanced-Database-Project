...
public class TransactionManager {
  ...
  public void read(int time, int transactionNumber, String dataName, boolean wait) throws Exception {
	...
	System.out.println(e);
      ...
      if (transactionCopy.transactionType.equals("readonly")) {
        if (currentSite.status.equals(Status.activeAndConsistent) || (currentSite.status.equals(Status.activeNotConsistent) && !dataToReadCopy.isReplicated) || dataToReadCopy.hasBeenWrittenToAfterRecovery) {
          ...
          for (int times = dataToReadCopy.modifiedTimes.size() - 1 ; times >= 0 ; times--) {
      		...
      		System.out.println("  > Transaction " + transactionNumber + " read " + dataName + " = " + dataToReadCopy.values.get(valueToReadIndex) + ".");
	...
	return;
    	...
    	if (sites[site].haveLock(transactionNumber, dataName) || (sites[site].isDataLocked(dataName) < 2 && (currentSite.status.equals(Status.activeAndConsistent) || !dataToReadCopy.isReplicated || dataToReadCopy.hasBeenWrittenToAfterRecovery))) {
    	  ...
    	  if (sites[site].getData(dataName).waitingQueue.isEmpty() || (!sites[site].getData(dataName).waitingQueue.isEmpty() && sites[site].getData(dataName).waitingQueue.get(0) != transactionNumber)) {
            sites[site].getData(dataName).waitingQueue.add(transactionNumber);
    	  }
          System.out.println("  > Transaction " + transactionNumber + " read " + dataName " = " + dataToReadCopy.values.get(dataToReadCopy.values.size() - 1) + ".");
          updateTransactionAccessInformation(transactionNumber, site, time);
          return;
    	  ...
    	  if (!wait && dataToReadCopy.isReplicated) {
    	    ...
    	    if (transactionCopy.arrivalTime < getTransaction(dataToReadCopy.waitingQueue.get(0)).arrivalTime) {
    	      // If transaction has an earlier arrival time than the one holding the lock.
    	      if (dataToReadCopy.waitingQueue.isEmpty() && (!dataToReadCopy.isReplicated || sites[site].status.equals(Status.activeAndConsistent) || (sites[site].status.equals(Status.activeNotConsistent) && dataToReadCopy.hasBeenWrittenToAfterRecovery))) {
    		// If the waiting queue for the lock is empty, get on line for the lock.
    	    	sites[site].getData(dataName).waitingQueue.add(transactionNumber);
    	    	
    	    	Operation operationToBuffer = new Operation(transactionNumber, transactionCopy.transactionType, "read", dataName);
    	    	...
    	    	int lastTransactionInQueue = dataToReadCopy.waitingQueue.get(dataToReadCopy.waitingQueue.size() - 1);
    	    	
    	    	if (transactionCopy.arrivalTime < getTransaction(lastTransactionInQueue).arrivalTime) {
        	  ...
        	  // line for the lock, abort.
    	    	  String abortReason = "transaction " + transactionNumber + " cannot get lock on " + dataName
    ...
    if (!foundDataInSearch) {
      ...
      String abortReason = "transaction " + transactionNumber + " cannot get a lock on " + dataName + " since " + dataName + " at all sites is held or is waiting on by older transactions.";
      abortTransaction(transactionNumber, abortReason);
      System.out.println("  > Transaction " + transactionNumber + " aborted!");
    }
  }
  ...
  public void write(int time, int transactionNumber, String dataName, int value) throws Exception {
	...
	if (sites[site].haveLock(transactionNumber, dataName) || sites[site].isDataLocked(dataName) == 0) {
	  ...
	  if (currentTransaction.arrivalTime < getTransaction(dataToWriteToCopy.waitingQueue.get(0)).arrivalTime) {
	    if (dataToWriteToCopy.waitingQueue.isEmpty()) {
	      sites[site].getData(dataName).waitingQueue.add(transactionNumber);
			  
	      Operation operationToBuffer = new Operation(transactionNumber, currentTransaction.transactionType, "write", dataName, value);
	      bufferOfOperations.add(operationToBuffer);
	      return;
	    } else {
	      if (sites[site].isAlreadyWaitingForData(transactionNumber, dataName)) {
		...
		return;
	      ...
	      int lastTransactionInQueue = dataToWriteToCopy.waitingQueue.get(dataToWriteToCopy.waitingQueue.size() - 1);
              if (getTransaction(transactionNumber).arrivalTime < getTransaction(lastTransactionInQueue).arrivalTime) {
                sites[site].getData(dataName).waitingQueue.add(transactionNumber);
		...
		String abortReason = "transaction " + transactionNumber + " cannot get lock on " + dataName + " since transaction " + lastTransactionInQueue + " had a lock on " + dataName + " and is older.";
		abortTransaction(transactionNumber, abortReason);
	    ...
	    String abortReason = "transaction " + transactionNumber + " cannot get a lock on " + dataName + " since transaction " + dataToWriteToCopy.waitingQueue.get(0) + " had a lock on " + dataName + " and is older.";
	    abortTransaction(transactionNumber, abortReason);
  ...
  public void dump(int siteNumber, String key) {
	...
	for (int value = 0; value < sites[siteNumber].listOfData.get(data).values.size(); value++) {
	  System.out.print("    " + sites[siteNumber].listOfData.get(data).values.get(value) + ", modified at time " + sites[siteNumber].listOfData.get(data).modifiedTimes.get(value));
	  if (value == sites[siteNumber].listOfData.get(data).values.size() - 1 && sites[siteNumber].listOfData.get(data).isTemporary) {
  ...
  public void end(int time, int transactionNumber) {
    ...
    if (getTransaction(transactionNumber).commitSuccess != null && getTransaction(transactionNumber).commitSuccess == false) {
    ...
    ArrayList<Transaction.AccessPair> accessInformation = getTransaction(transactionNumber).getAccessInformation();
    ...
    for (int record = 0; record < accessInformation.size(); record++) {
      ...
      if (sites[siteAccessed].status.equals(Status.failed) || ((sites[siteAccessed].lastRecoverTime != null) && timeAccessed < sites[siteAccessed].lastRecoverTime)) {
  ...
  public void cleanUpAbortedTransaction(int transactionNumber) {
    ...
    for (int site = 0; site < numberOfSites; site++) {
      for (int data = 0; data < sites[site].listOfData.size(); data++) {
  	if (!sites[site].listOfData.get(data).waitingQueue.isEmpty() && sites[site].listOfData.get(data).waitingQueue.get(0) == transactionNumber) {
   ...
   */
  public void updateTransactionAccessInformation(int transactionNumber, int siteNumber, int accessTime) {
    for (Transaction transaction : transactions) {
      if (transaction.transactionNumber == transactionNumber) {
